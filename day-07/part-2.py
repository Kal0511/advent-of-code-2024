...
def is_valid(equation, total, i):
    ...
    if total - equation[i] >= equation[1] and is_valid(equation, total - equation[i], i - 1):
        return True
    ...
    if (
            i >= 2 and
            len(whole) > len(curr) and
            whole.endswith(curr) and
            is_valid(equation, int(whole[:-len(curr)]), i - 1)
    ):
        return True
    return False
